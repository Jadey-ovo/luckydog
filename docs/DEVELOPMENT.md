...
迁移 0004 把旧版 7 天 expires 减去 6 天，保留原创建基准；旧随机 winner ID 按活动内唯一姓名映射回参与者 ID，已开奖结果获得一次 90 秒迁移租约，仍打开的发起页会继续续租。更新原子报名触发器，拒绝过期、租约失效、已开奖的插入。`migrations/` 和 `drizzle/` 同步追加，旧文件保持不变。
