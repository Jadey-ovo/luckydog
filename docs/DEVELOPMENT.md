...
Sites 邀请的 `expires` 固定为创建时间 + 86400000 ms；`join_expires` 独立按 5/10/30 分钟计算。未开奖房间的 `active_until` 是 90 秒发起人租约，失去续期只转为 interrupted，保留可查询状态与本人信息，不立即删除。完成开奖将租约设为 NULL；所有续期、重新开放与重抽路径均不能延长 expires。重置通过 PATCH archive 标记 archived=1、关闭报名并释放租约，不删除记录；旧 DELETE 也按归档处理，避免旧客户端破坏历史。到期 prune 才级联删除参与者与 room_draws。
...
管理令牌仅在内存中，刷新不恢复管理。未开奖连接中断不可续命，需返回重新发起。已开奖关闭页面仍能查询至原 24 小时期满。服务端成功保存之前不能承诺分享成功，客户端提供同步重试，重试使用同一开奖结果。未完成同步即退出，参与者不会看到未保存的本地结果。
