...
渲染进程不开启 Node，启用沙箱、上下文隔离和 Web 安全。预加载脚本只暴露在线邀请请求方法，主进程严格校验请求路径、方法、管理凭据与大小，并固定连接 Luckydog Sites 域名；不接受渲染进程提供的目标网址。其他远程网络请求、新窗口和页面跳转仍由主进程拦截。应用包仅收录 `dist/`、`electron/`、`package.json`，构建依赖不进入运行包。
