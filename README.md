...
网页版手动名单只在当前页面内存中保留，刷新或关闭即清空。在线邀请和分享结果仅在发起页面保持打开时临时保存；确认名单、返回或关闭页面后会删除并失效，关闭通知未送达时由约 90 秒的在线租约兜底。桌面版使用本机的 `localStorage` 保存名单，它不是加密存储，共用电脑上请用完清空。应用不会把名字写进网址，也不会发送姓名或结果给 GitHub。
...
Sites 复用 `worker/index.mjs` 的报名、去重、管理权限和结果接口，网页界面保持一致。额外的适配包括 `.openai/hosting.json` 中的 `DB` 声明、Drizzle 迁移，以及 `worker/sites.mjs` 的托管入口。邀请在确认名单时立即删除；关闭发起页面时，浏览器会发送销毁请求。浏览器关闭请求无法得到平台级百分百保证，因此服务另用短时在线租约兜底：发起页面停止活动后约 90 秒内，邀请和结果链接即使未成功删除也会失效。后台记录在后续 API 访问时清理。
