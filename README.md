...
2. 实时查看参与名单；到期自动截止，也可以确认后提前截止。原截止时间之前可恢复报名。
3. 核对名单、设置中奖名额，点击「开始抽奖」。开奖时会播放礼炮音效与彩纸动画，原邀请链接自动切换为结果页。结果页可以保留名单继续抽奖，或清空数据开始新活动。

在线版目前以邀请报名作为名单入口。桌面版保留手动名单输入，支持换行、逗号、分号和空白分隔。同一轮不会重复中奖；选择“继续抽奖”时会重新从完整名单独立抽取，因此可能与上一轮中奖者重复。移除参与者、继续抽奖和清空活动都有影响说明或二次确认。
...
| 你的电脑 | v1.2.0 安装包 | 打开方式 |
| --- | --- | --- |
| Windows 10 / 11，64 位 x64 | [下载 EXE](https://github.com/Jadey-ovo/luckydog/releases/download/v1.2.0/Luckydog-1.2.0-win-x64.exe) | 双击，按安装向导操作 |
| Mac，Apple 芯片 M 系列 | [下载 DMG](https://github.com/Jadey-ovo/luckydog/releases/download/v1.2.0/Luckydog-1.2.0-mac-arm64.dmg) | 打开后拖入「应用程序」 |
| Mac，Intel 芯片 | [下载 DMG](https://github.com/Jadey-ovo/luckydog/releases/download/v1.2.0/Luckydog-1.2.0-mac-x64.dmg) | 打开后拖入「应用程序」 |

[全部版本与更新说明](https://github.com/Jadey-ovo/luckydog/releases) · [下载文件校验值](https://github.com/Jadey-ovo/luckydog/releases/download/v1.2.0/SHA256SUMS.txt)
...
在线邀请和分享结果仅在发起页面保持打开时临时保存。提前截止或自然到时后，参与页显示“报名已截止”、当前访问者自己的报名用户名和等待开奖提示；发起人清空活动、刷新或关闭页面后，页面才显示“该活动已失效”。清空会立即删除活动，关闭通知未送达时由约 90 秒的在线租约兜底。桌面版使用本机的 `localStorage` 保存名单，它不是加密存储，共用电脑上请用完清空。应用不会把名字写进网址，也不会发送姓名或结果给 GitHub。
...
Sites 复用 `worker/index.mjs` 的报名、去重、管理权限和结果接口，网页界面保持一致。额外的适配包括 `.openai/hosting.json` 中的 `DB` 声明、Drizzle 迁移，以及 `worker/sites.mjs` 的托管入口。邀请到期或发起人提前截止后不再接收报名，并保留“报名已截止”状态和本人报名信息供参与者等待开奖。发起人点击“清空返回”或关闭页面时，浏览器会发送销毁请求；若关闭请求未能送达，短时在线租约会在约 90 秒内让邀请和结果链接失效。
...
邀请链接可设置 5、10、30 分钟有效期，默认 5 分钟。到期或提前截止后，参与页显示“报名已截止”、本人报名用户名和等待结果提示；提前截止后可在原截止时间之前继续报名，原截止时间到了便不再显示返回入口。开奖后，原二维码和链接直接显示最新中奖名单。继续抽奖保留当前名单并可能重复上一轮中奖者；清空返回会二次确认，删除邀请、名单和结果并让原链接立即失效。关闭或刷新发起页面时也会请求删除活动，邀请与结果一起失效。为处理浏览器来不及发送关闭请求的情况，发起页面持续续期，停止续期后约 90 秒自动失效。D1 的平台备份、恢复历史和托管访问日志有各自的保留策略，不等于应用记录删除。发起页面的管理令牌仅保留在内存中。报名通过 HttpOnly Cookie 限制同一浏览器一次，并禁止重复用户名。数据库在写入时检查截止时间、人数上限和唯一性，防止多个请求同时报名绕过去重。

在线邀请活动开奖时会自动把中奖名单写回活动记录。未使用邀请活动时，仍需点击“生成分享链接”才会上传中奖名单。桌面版不提供在线分享。
