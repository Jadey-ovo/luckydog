...
export function ProductManual() {
  ...
  return <>
    ...
    {open&&createPortal(<div className="modal-backdrop"><div className="dialog manual-dialog" role="dialog" aria-modal="true" aria-labelledby="manual-title">
      ...
      <div className="manual-notes"><strong>查询与清空</strong><p>邀请链接从活动创建起 24 小时内可查询状态与本人结果，报名仍按所选时长截止。开奖后关闭或刷新发起页面不会删除已同步结果，但无法恢复发起人的管理权限。</p><p>未开奖时请保持发起页面在线；连接中断约 90 秒后活动中断，不再接受报名或开奖。点击“清空返回”需要二次确认，只重置发起页并结束当前活动。历史链接和本人每轮记录保留至原查询期限。新活动使用新链接与二维码。无人报名到期后可返回重新发起。</p><p>请用报名时的浏览器查询，清除 Cookie 或更换设备无法识别本人。创建满 24 小时后链接失效，数据由服务自动清理。用户名与结果临时保存在 Sites 托管服务；Windows / Mac 桌面版手动名单只保存在本机。</p></div>
