...
export function ProductManual() {
  ...
  return <>
    ...
    {open&&createPortal(<div className="modal-backdrop"><div className="dialog manual-dialog" role="dialog" aria-modal="true" aria-labelledby="manual-title">
      ...
      <div className="manual-notes"><strong>查询与清空</strong><p>邀请链接从活动创建起 24 小时内可查询状态与本人结果，报名仍按所选时长截止。每次开奖只新增一轮结果，发起人仍可继续抽奖。</p><p>桌面版的抽奖界面与随机计算在本机运行；创建扫码邀请需要联网，报名名单和每轮结果会临时同步到固定的 Luckydog Sites 服务。</p><p>点击“清空返回”并确认后活动立即结束；关闭或刷新发起页会停止在线续期，约 90 秒后结束。结束不会删除已同步结果，原链接仍保留本人每轮记录至查询期限。刷新后无法恢复发起人的管理权限。</p><p>未开奖时请保持发起页面在线；连接中断后活动会中断，不再接受报名或开奖。新活动使用新链接与二维码。请用报名时的浏览器查询，清除 Cookie 或更换设备无法识别本人。创建满 24 小时后链接失效，数据由服务自动清理。</p></div>
