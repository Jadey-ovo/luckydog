...
export function Toast({message,onClose,status='error',revision=0,dismissible=true}:{message:string;onClose:()=>void;status?:'success'|'error'|'info';revision?:number;dismissible?:boolean}) {
 const close=useRef(onClose);close.current=onClose;
 useEffect(()=>{if(!message)return;const timer=setTimeout(()=>close.current(),4500);return()=>clearTimeout(timer);},[message,revision]);
